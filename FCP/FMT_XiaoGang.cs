    ...
    class FMT_XiaoGang:FormatCollection
    {
        ...
        private ResultType OPD_Process()
        {
            try
            {
                ...
                for (int r = 0; r <= PatientInfo.Count - 2; r++)
                {
                    ...
                    if (Settings.EN_FilterMedicineCode && !MedicineCodeGiven_L.Contains(MedicineCodeJudge))
                        continue;
                    ...
                    if (!GOD.Is_Admin_Code_For_Combi_Created($"S{AdminCode_S}"))
                    {
                        ...
                        LoseContent = $"{FullFileName_S} 在OnCube中未建置此種包頻率 S{AdminCode_S} 的頻率";
        ...
        private ResultType OPD_Logic()
        {
            ...
            try
            {
                ...
                if (a != 0)
                {
                    yn = oncube.XiaoGang_OPD(MedicineName_L, MedicineCode_L, AdminCode_L, PerQty_L, SumQty_L, StartDay_L, FileNameOutputCount, ID1_L,
        ...
        private ResultType UD_Process()
        {
            try
            {
                ...
                for (int r = 0; r <= PatientInfo.Count - 2; r++)  //將藥品資料放入List<string>
                {
                    ...
                    if (ecd.GetString(ATemp, 46, 8).Trim() == "8DH" & Settings.StatOrBatch == "長期")
                    {
        ...
        private ResultType UD_Logic()
        {
            ...
            try
            {
                ...
                Class_L, WareHouse, IsStat_B, AutoNumber, NumberDetail_L, NursingSattionNumber, EffectiveDate, MedicalUnit_L, Settings);
                if (yn)
                {
                    if (IsStat_B)
